...
public interface OrderRepository extends  CrudRepository<OrderDetail, Long>
{
    ...
    @Query(value = "SELECT * FROM ORDER_DETAIL WHERE USERNAME = ?1 ORDER BY CREATEAT DESC", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM ORDER_DETAIL WHERE POSTER = ?1 ORDER BY CREATEAT DESC", nativeQuery = true)
